...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("does not recover a parent issue while delegated child work remains open", async () => {
    const { companyId, agentId, issueId } = await seedStrandedIssueFixture({
      status: "in_progress",
      runStatus: "failed",
      runErrorCode: "process_lost",
    ...
    await db.insert(issues).values({
      ...
      companyId,
      parentId: issueId,
      title: "Delegated child work still in progress",
      status: "in_progress",
      priority: "medium",
  ...
  it("DGG-5210 AC-3: auto-marks a recovery issue done when its latest retry run succeeded productively", async () => {
    ...
    expect(result.dispatchRequeued).toBe(0);
    expect(result.continuationRequeued).toBe(0);
    // Auto-done retires the recovery issue itself, and because that recovery
    // issue was the source's terminal `blocks` blocker, the liveness terminal-
    // blocker cleanup removes the now-stale relation in the same tick and
    // surfaces the source in issueIds too.
    expect(result.terminalBlockerRelationsRemoved).toBe(1);
    expect(result.issueIds).toEqual([issueId, sourceIssueId]);
    ...
    // Source keeps its own status (`blocked`); only its stale terminal-blocker
    // relation to the completed recovery issue is cleaned up.
    const source = await db.select().from(issues).where(eq(issues.id, sourceIssueId)).then((rows) => rows[0] ?? null);
    expect(source?.status).toBe("blocked");
    expect(source?.blockedByIssueIds ?? []).not.toContain(issueId);
  });
